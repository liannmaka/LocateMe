...
<template>
  ...
  <div v-if="!showFriendModal" class="cursor-pointer p-2 z-40 absolute right-2 lg:right-12 bg-white top-[130px] hover:scale-105 shadow-md rounded-full" @click="showFriendModal=true">
  ...
  <div v-else class="rounded-md items-start justify-start w-full flex-col transition ease-in-out delay-150 duration-300 absolute z-40 lg:right-0 lg:w-[30rem] h-screen bg-white flex p-2">
    ...
    <section class="flex items-center justify-start bg-white w-full gap-5 p-2">
      ...
      <button @click="showFriendModal=false" class="p-1 text-sm font-bold">Cancel</button>
...
<!--      </svg>-->
    ...
    <section class="w-full flex flex-col gap-10 overflow-scroll p-2">
...
<!--      </ul>-->
      ...
      <div class="w-full rounded-md shadow-md ">
        <h3 class="py-3 p-3 text-md text-neutral-900">Friends</h3>
      ...
      <div class="w-full rounded-md shadow-md">
        <h3 class="py-3 p-3 text-md text-neutral-900">Invites</h3>
        <section  class="w-full p-2 flex flex-col h-auto justify-center">
      ...
      <div class="w-full rounded-md shadow-md">
        <h3 class="py-3 p-3 text-md text-neutral-900">Request</h3>
        <section  class="w-full p-2 flex flex-col h-auto justify-center">
  ...
  <section class="flex items-center z-30 absolute bg-[#fafafa] flex-col p-1  bottom-[70px] lg:bottom-10 rounded-t-3xl lg:rounded-none  lg:shadow-lg lg:justify-start lg:right-5 w-full lg:w-auto gap-1">
    <div class="w-full bg-[#fcfcfc] absolute hidden transition-all duration-300" id="friend">
      ...
      <div class="flex items-center justify-start gap-2 w-full">
        <img :src="currentFriend?.img" alt="profile_img" class="flex w-16 h-16 rounded-full items-center justify-center gap-1 bg-white cursor-pointer p-3 hover:bg-gray-100"  />
        <div class="flex flex-col items-start justify-start gap-1 w-auto">
          <p class="text-xs text-gray-500">{{ currentFriend?.name }}</p>
          <p class="text-xs text-gray-500">Last active {{ currentFriend?.last_seen }}</p>
        </div>
      </div>
    </div>
    <div class="flex bg-[#fafafa] overflow-scroll hide-scroll-bar">
      ...
      <img v-for="(i, index) in  friends" :key="index" @click="trackFriend(i)" :src="i.img" :alt="index" class="flex w-16 h-16 rounded-full items-center justify-center gap-1 cursor-pointer p-3 hover:bg-gray-100"  />
